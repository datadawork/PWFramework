...
namespace PWFrameWork
{
    public class ASM
    {
        //Объявляем класс для работы с памятью, который будет выделять память
        private MemoryWork memory;

        //Конструируем класс
        public ASM(MemoryWork memory_work)
        {
        ...
        {
            this.Asmcode = this.Asmcode + "8902";
        }
        ...
        public void Mov_ESI_DWORD_Ptr_EAX_Add(int addre)
        {
            if ((addre <= 127) && (addre >= -128))
            {
                this.Asmcode = this.Asmcode + "8B70" + intTohex(addre, 2);
            }
            else
            {
                this.Asmcode = this.Asmcode + "8BB0" + intTohex(addre, 8);
            }
        }
        ...
        public void Mov_EAX(float addre)
        {
            ...
            this.Asmcode = this.Asmcode + "B8" + intTohex(intVal, 8);
        }
        ...
        {
            this.Asmcode = this.Asmcode + "E8" + intTohex(((TargetCall - OldAddress) - 10), 8);
        }

        public void Call_Adr(int addre)
        {
            this.Asmcode = this.Asmcode + "E8" + intTohex(addre, 8);
        }
        ...
        public void RunAsm()
        {
            ...
            WinApi.WriteProcessMemory(memory.OpenedProcessHandle, memory.FuncAllocMemory, Asm, Asm.Length, out tmpInt);
            IntPtr tmpIntPtr;
            //запускаем этот байт код в отдельном потоке
            WinApi.CreateRemoteThread(memory.OpenedProcessHandle, IntPtr.Zero, 0, memory.FuncAllocMemory, IntPtr.Zero, 0, out  tmpIntPtr);
            //очищаем эту память 
